...
ones). Reference: @$HOME/.config/opencode/gsd-core/references/edge-probe.md.
...
# — NOT the consuming project's git root — falling back to git toplevel / $HOME/.config/opencode.
...
EDGE_PROBE_JS=$(for _c in \
  ...
  "$HOME/.config/opencode/gsd-core/bin/lib/edge-probe.cjs" \
  "$HOME/.config/opencode/bin/lib/edge-probe.cjs"; do
  [ -f "$_c" ] && { echo "$_c"; break; }
done)
...
requirement list). Reference: @$HOME/.config/opencode/gsd-core/references/prohibition-probe.md — the
...
Use the SPEC.md template from @$HOME/.config/opencode/gsd-core/templates/spec.md.
